...
author: "Karthikeyan Murugesan, Greg Gibson (http://cig.gatech.edu)
...
Before proceeding towards any downstream analysis,  the researcher has to inspect the
gene expression  data to understand the genetic,  environmental,  population,  technical and
confounding factors which can bias the gene expression values.This
needs to be studied  to ensure that the contributions of non biological variables to any
observed biological signal is accounted for.
  We suggest using a dynamic workflow to tackle the above mentioned problem as there is no 
'one fits for all' data analysis approach that works for every  experiment.This workflow has been 
designed keeping this in mind, mainly using analyses  methods like the supervised normalization of microarrays,  
surrogate variable analysis and the principal variance component analysis amongst others.

The data is based on the paper by [@dataRef].
Link to the GEO data set http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE49925

# Package Content

The workflow package contains five functions that will be frequently used prior to any empirical analysis in order to inspect batch effects that may affect biological inferences.These are:
...
2. CAD_Exptdsgn.csv - file containing the phenotype information (100 samples across 17 covariates )
...
An ExpressionSet Class[@Ref1] is a Biobase data structure that is used to conveniently store experimental information and associated meta data, all in  one place. Here an object of the the ExpressionSet Class is being created, which stores the gene expression values and the phenotype data.

```{r eval=TRUE}
inpData <- expSetobj(exprs, covrts)
```

## C. Principal Variance Component Analysis(PVCA)[@Ref2] of the un-normalized data

PVCA estimates the variance in the expression data set  due to each of the given covariates and
...
Here PVCA is used to estimate the variance due to various CAD covariates like BMI, Ethn, CAD, Rin 
amongst others.For example from the PVCA output graph below , 'Ethnicity' is the covariate of major 
influence, explaining up to 11.7% of the variance in the gene expression data.
...
## D. Surrogate Variable Analysis[@Ref3]
...
The user provides a biological variable based on which the surrogate variables are generated. These are then appended as 
new covariates to the existing list of covariates.For a thorough introduction to SVA, refer to the paper by [@Ref3]
...
surrogate variable is a manifestation or a function of one or more existing covariates.Surrogate variables may not be a significant threat if they are associated with existing covariates but could become a very important factor if they are totally independent of others. For this purpose a generalized linear model is run where the identified surrogate variables are modeled as a function of the existing covariates  
...
## G. Supervised normalization of Microarrays(SNM)[@Ref4]
...
Choose the biological variables, the adjustment variables and the intensity dependent variables intelligently
based on effect sizes as seen from PVCA.Here we are removing the effects of the study, sv1 and sv2 covariates
SNM is typically run for 5-20 iterations.In the current workflow all SNM functions are run for 5 iterations.
...
This post SNM PVCA reflects how the effect sizes of the covariates span out after removing the effects of the adjustment variables.SNM brings down their effect to a minimal possible value while trying to preserve the variation due to the biological signals.
...
Note the increase in the effect sizes of the 'BMI', 'Rin', 'Ethn' and the 'CAD' covariates (These weren't added as adjustment variables in 
the prior SNM step).
...
The sample workflow aims to analyze the gene expression data used in the Coronary Artery Disease study by [@dataRef], determine its validity for downstream analysis and normalize it if necessary.      
...
from the PVCA analysis (figure 1). This effect has to be removed to obtain clean process-able data.
...
From the initial PVCA we observe a batch effect because of the 'study' covariate and we normalize the expression values to remove any bias caused by 'study'. This is a safe approach to proceed with and is generally carried out by researchers . But there's always more to it than meets the eye.
```{r eval = TRUE, echo=FALSE}
bv <- c("CAD") ## Chose your biological variable covariates
...
From Workflow step D we are able to identify two hidden covariates or surrogate variables for our data
...
simultaneously increase the residual from 36.9% to 72.1% which is big jump, hence effectively retrieving the lost biological signals  
